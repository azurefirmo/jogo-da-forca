...
document.addEventListener("keydown", (evento) => {
  ...
  if (isJogando && alfabeto.indexOf(letraTeclada) >= 0) {
    if (
      secreta.indexOf(letraTeclada) >= 0 &&
      quantErros < 10 &&
      tracinhosAux(secretaAux.length) != secretaAux
    ) {
      ...
      if (
        listaErradas.indexOf(letraTeclada) < 0 &&
        quantErros < 10 &&
        tracinhosAux(secretaAux.length) != secretaAux
      ) {
...
function ocultarSecaoBotoes() {
  document.querySelector(".secao-botoes").classList.add("ocultar");
}
function mostrarSecaoBotoes() {
  document.querySelector(".secao-botoes").classList.remove("ocultar");
}
